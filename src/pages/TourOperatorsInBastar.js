...
import { TourOperators } from "../Path/PackageCategories";
...
function TourOperatorsInBastar() {
  ...
  const getPackages = async () => {
    try {
      const res = await fetch(
        `${API_PATH}/api/v1/packages/list?category=${TourOperators}`
      );
        ...
        <div className="d-flex justify-content-between">
          <div>
            <h2 className="package__title mb-5">
              <span>Tour Operators In Bastar</span>
            </h2>
          </div>
...
export default TourOperatorsInBastar;
